...
// Previous(CDKS19) MultAndRelin algorithm

func (ks *KeySwitcher) PrevMulAndRelin(op0, op1 *Ciphertext, rlkSet *RelinearizationKeySet, ctOut *Ciphertext) {
	level := ctOut.Level()
	...
	for id0 := range idset0.Value { // id0: i
		for id1 := range idset1.Value { // id1: j
			...
			ks.ExternalProduct(level, ks.polyQPool[2], d, ks.polyQPool[0]) //pool[0]: c_i,j ext d
			ringQ.AddLvl(level, ks.polyQPool[0], ctOut.Value[id1], ctOut.Value[id1])

			ks.ExternalProduct(level, ks.polyQPool[2], b, ks.polyQPool[0]) //pool[0]: c_i,j ext b
			ks.ExternalProduct(level, ks.polyQPool[0], v, ks.polyQPool[1]) //pool[1]: b ext v
			ks.ExternalProduct(level, ks.polyQPool[0], u, ks.polyQPool[2])

			ringQ.AddLvl(level, ctOut.Value["0"], ks.polyQPool[1], ctOut.Value["0"])
			ringQ.AddLvl(level, ctOut.Value[id0], ks.polyQPool[2], ctOut.Value[id0])
